...
async fn test_magic_dns_server_instance() {
    #[cfg(target_os = "macos")]
    {
        eprintln!("skip magic_dns_server_instance on macOS due to TUN privilege requirements");
    }
    #[cfg(not(target_os = "macos"))]
    {
...
async fn test_magic_dns_runner() {
    #[cfg(target_os = "macos")]
    {
        eprintln!("skip magic_dns_runner on macOS due to TUN privilege requirements");
    }
    #[cfg(not(target_os = "macos"))]
    {
        let tun_ip = Ipv4Inet::from_str("10.144.144.10/24").unwrap();
